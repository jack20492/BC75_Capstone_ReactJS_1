          ...
          <div key={index}>
            <a target="blank" href={item.url}>
              <img
                src={item.img}
                alt=""
                className="rounded-lg cursor-pointer h-72 w-full object-cover"
              />
            </a>
            <a target="blank" href={item.url}>
    ...
    return (
      <div className="p-0 m-0 grid gap-7">
        {itemData.map((item, index) => (
          <div
            key={index}
            className="flex flex-row justify-start md:justify-between gap-3"
          >
            <a target="blank" href={item.url} className="basis-auto">
              <img
                src={item.img}
                alt=""
                className="rounded-lg cursor-pointer h-20 w-20 object-cover"
              />
            </a>
            <p
              target="blank"
              href={item.url}
              className="basis-4/5 text-gray-700 text-justify"
            >
              {item.title}
            </p>
          </div>
      ...
      const filteredData = totalSearchFilm.heThongRapChieu
        .map((heThongRap) =>
          heThongRap.cumRapChieu.filter(
            (cumRap) => cumRap.maCumRap === searchCinema
          )
        )
        .flat();
      const arr = [];
      filteredData.map((item) => {
        item.lichChieuPhim.map((itemChild) =>
          arr.push({
            ...
            label: moment(itemChild.ngayChieuGioChieu).format(
              "DD-MM-YYYY ~ HH:mm"
            ),
          })
        );
    ...
    <>
      ...
      <div className="">
        ...
        <div className="container grid grid-cols-1 lg:grid-cols-4 gap-3 pt-[80px]">
          ...
          <button
            disabled={!chosenTime}
            className={`w-full mx-auto text-white bg-red-500 ${
              chosenTime ? "opacity-100" : "opacity-50"
            } rounded ${chosenTime && "hover:bg-red-800"} duration-300 ${
              chosenTime ? "cursor-pointer" : "cursor-not-allowed"
            }`}
              ...
              <div className="flex justify-center items-center text-center md:text-justify">
                <div className="space-y-12">
                  <h1 className="font-bold text-3xl">
                    Ứng dụng tiện lợi dành cho dân yêu điện ảnh
                  </h1>
                  <p>
                    ...
                    {Array.from({ length: 15 }, (_, i) => i + 1).map(
                      (index) => (
                        <img
                          ...
                          src={`https://movie-booking-project.vercel.app/img/mobile/slide${
                            index + 1
                          }.jpg`}
                          alt=""
                        />
