...
export default function ListMovie({ movieArr }) {
  ...
  // Function to parse trailer URL and extract video ID
  const parseTrailerUrl = (trailer) => {
    let videoId = "";
    if (trailerYoutube.test(trailer) || trailer.includes("embed")) {
      // Extract video ID from YouTube trailer
      const parts = trailer.split("/");
      videoId = parts[parts.length - 1];
    } else {
      // Extract video ID from URL search parameters (e.g. v=xxxxx)
  ...
  const handleChooseTrailer = (trailer) => {
    const videoId = trailerUrlRegex.test(trailer)
      ? parseTrailerUrl(trailer)
      : parseTrailerUrl(defaultTrailer);
          ...
          <div key={index} className="relative group">
            ...
            >
              ...
              <NavLink className="text-white" to={`/detail/${item.maPhim}`}>
                <button className="py-3 mt-3 w-full mx-auto text-white bg-red-500 rounded hover:bg-red-800 duration-300">
                  Book tickets
                </button>
              </NavLink>
            </Card>
